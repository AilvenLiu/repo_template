...
declare the identical version; disagreement is a hard gate failure. Read the
Python runtime version from installed metadata rather than maintaining a static
`__version__` mirror. An ordinary release-content PR updates both manifests
and `CHANGELOG.md` together.

Release naming is derived from the manifests, never typed by hand:
...
re-proves one strictly increasing commit changing only the root
`CMakeLists.txt` and `pyproject.toml` version fields; it intentionally does
not run the full build. Do not use this fast path if the project retains another
manually maintained version mirror.
