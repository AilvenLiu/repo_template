...
Codex-native best-effort skills included in generated Python repos:
- `build` via `bin/agent-build <setup|compile|test|full|doctor|clean>`
- `navigate` via repo-native `rg`-first exploration
- `python-env-setup` via `bin/agent-python-env-setup <diagnose|fix|verify>`
...
- `dependency` via `bin/agent-dependency add <pkg> [version] [--dev]`
...
- NEVER use direct `pip install` in project workflows — use `/dependency` or
  `bin/agent-dependency`
- NEVER run `python`/`python3` directly for application/test workflows —
  use `poetry run python`
- Agent infrastructure commands (`bin/agent-*`, `.ai/tools/*`) are exempt
  when using the controlled wrappers
...
| Add dependency | `dependency` | `bin/agent-dependency add <pkg> [ver] [--dev]` |
