...
- MUST use `/dependency add <package> [version]`
- MUST NOT use system package managers or manual `conan install` directly

### Before every commit
1. MUST run `/pre-commit validate` and confirm it passes (clang-format, clang-tidy, cppcheck, build)
...
|--------|---------|-----------|
| Add library | `/dependency add <pkg>` | `apt install`, `brew install`, `conan install` directly |
...
- `.claude/constraints/cpp/dependencies.md`
- `.claude/constraints/cpp/forbidden-practices.md`
- `.claude/constraints/cpp/error-handling.md`
- `.claude/constraints/cpp/static-analysis.md`
- `.claude/constraints/cpp/testing.md` (when test files modified)
- `.claude/constraints/cpp/formatting.md` (when .cpp/.hpp files modified)
- `.claude/constraints/cpp/memory-safety.md` (when .cpp/.hpp files modified)
- `.claude/constraints/cpp/cuda.md` (when .cu/.cuh files modified)
- `.claude/constraints/cpp/cmake.md` (when CMakeLists.txt modified)
