...
- bin/agent-roadmap create produces complete phase folders with all 4 files
...
def project_root(tmp_path: Path) -> Path:
    ...
    init = _run(["bash", "bin/agent-init", "--platform", "claude"], target)
    # Audit is expected to fail in sandboxed test env (no Context7).
    # We only care about the constraint-loading & state-file path.
    assert "common/agentic-team" in init.stdout
    _force_audit_pass(target)
    return target


def test_init_loads_agentic_team_constraint(project_root: Path) -> None:
    # The fixture asserted constraint loading; re-run to capture the printed body.
    result = _run(["bash", "bin/agent-init", "--platform", "claude"], project_root)
    assert "common/agentic-team" in result.stdout
...
def test_roadmap_validate_passes_for_freshly_created_phase(project_root: Path) -> None:
    ...
    result = _run(
        ["bash", "bin/agent-roadmap", "validate", "phase-0-baseline"],
        project_root,
    )
