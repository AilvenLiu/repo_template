...
def _is_explicit_non_override(
    ...
    if any(phrase in prefix for phrase in ("does not", "do not", "cannot", "can't")):
        return True
    return previous_line.rstrip().lower().endswith(("does not", "do not", "cannot", "can't"))
...
def scan_text(path: Path, text: str) -> list[InstructionSafetyViolation]:
    ...
    for line_number, line in enumerate(lines, start=1):
        for rule in _RULES:
            ...
            if rule.name == "repository-overrides-platform" and _is_explicit_non_override(
                line, previous_line, match
            ):
                continue
            violations.append(
