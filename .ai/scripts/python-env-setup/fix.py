...
from diagnose import EnvironmentDiagnostics, Issue
...
class EnvironmentFixer:
    ...
    def fix_virtual_env(self) -> bool:
        ...
        if zshrc.exists():
            ...
            with open(zshrc, "w") as f:
                f.write("# CRITICAL: Unset system-set VIRTUAL_ENV to prevent Poetry conflicts\n")
                f.write("unset VIRTUAL_ENV\n")
    ...
    def check_poetry_venv(self) -> bool:
        ...
        try:
            ...
            if not self.confirm("  Remove external venv and configure for in-project venvs?"):
                print("  [SKIP] Skipped by user")
                return False
    ...
    def fix(self) -> bool:
        ...
        if not self.diagnostics.has_critical_issues():
            ...
            print("  .ai/bin/agent-python-env-setup diagnose")
            return True
        ...
        print("5. Verify: .ai/bin/agent-python-env-setup verify")
        print()
