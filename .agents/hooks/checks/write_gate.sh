#!/bin/bash
# Write Gate: delegates init/audit policy to .agents/scripts/policy_gate.py and keeps file guards.
...
if [ -z "$FILE_PATH" ]; then
    python3 "$REPO_ROOT/.agents/scripts/policy_gate.py" --op mutate --context '{}' >/dev/null
    exit 0
fi
...
python3 "$REPO_ROOT/.agents/scripts/policy_gate.py" --op mutate --context "$CONTEXT_JSON" >/dev/null
...
if echo "$FILE_PATH" | grep -qE '(^|/)pyproject\.toml$|(^|/)CMakeLists\.txt$|(^|/)cmake/Dependencies\.cmake$|(^|/)cmake/Options\.cmake$'; then
    echo "WARNING: This file can affect dependency/build ownership. Run .agents/bin/agent-check-constraints before final response." >&2
    exit 0
fi
