...
This directory contains the canonical hook implementations and regression tests. Thin adapters under `.claude/hooks/` and `.codex/hooks/` translate native lifecycle events into the shared policy gate.
...
| `.agents/bin/agent-init` | ALLOW | Exact init invocation |
| `.agents/bin/agent-init --verbose` | ALLOW | Exact init invocation with verbose |
| `.agents/bin/agent-init --platform claude` | ALLOW | Exact init invocation for Claude |
...
```
.agents/bin/agent-init && echo hacked > .agents/session_state.json
```
...
| `apt install lib*-dev` | BLOCK | Enforces the CMake/CPM dependency workflow for C++ libraries |
| `brew install <non-toolchain>` | BLOCK | Enforces the CMake/CPM dependency workflow for C++ libraries |
| `rm -rf src/lib/include/tests/.claude` | BLOCK | Protects source and config |

`bash_gate.sh` delegates policy decisions to `.agents/scripts/policy_gate.py --op bash`,
...
`write_gate.sh` uses `.agents/scripts/policy_gate.py --op mutate` for shared init/audit
...
4. Test with: `echo '{"tool_name":"Bash","tool_input":{"command":"<cmd>"}}' | .agents/hooks/pre_tool_use.sh`
...
```bash
bash .agents/hooks/tests/test_security_bypasses.sh
```
