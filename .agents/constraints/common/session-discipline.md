...
1. Run the platform's session initialization procedure
   (e.g., `/init` for Claude Code, `.agents/bin/agent-init --platform codex` for Codex)
...
`.agents/capabilities.yml` as the canonical manifest and checks:
