...
**ABSOLUTELY FORBIDDEN**: Installing packages directly with pip or creating a manual virtual environment.

Use Poetry through the repository workflows:

```bash
.agents/bin/agent-python-env-setup verify
.agents/bin/agent-dependency add package-name
```

### 2.2 Dependency Management

**FORBIDDEN**:
- Direct `pip`, `pip3`, or `python -m pip` dependency installation
- Manual `venv` creation or activation
- Direct `poetry add` or dependency-file edits that bypass `.agents/bin/agent-dependency`
- Unreviewed or undocumented production dependency changes
...
# ALLOWED: Use a proper package layout declared in pyproject.toml
# and install it through the repository Poetry workflow.
```
