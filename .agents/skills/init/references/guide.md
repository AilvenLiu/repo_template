...
- Run the capability audit from `.agents/capabilities.yml`
...
```bash
.agents/bin/agent-init --platform claude --verbose
```

## Constraint Files

Constraints are organized by topic in `.agents/constraints/`:
...
[READ] .agents/constraints/common/git-workflow.md
[READ] .agents/constraints/common/roadmap-awareness.md
[READ] .agents/constraints/python/formatting.md
[READ] .agents/constraints/python/type-checking.md
[READ] .agents/constraints/python/testing.md
...
- Manually read constraint files from `.agents/constraints/`

### Roadmap Check Fails

If roadmap checking fails:
- Ensure `/roadmap` skill is installed
- Check that `.agents/scripts/roadmap/check.py` exists
...
1. Modify `.agents/bin/agent-init` for detection logic
2. Update `.claude/skills/init/SKILL.md` for documentation
3. Add new constraint files to `.agents/constraints/` as needed
