#!/usr/bin/env python3
"""Scan source files for patterns forbidden by .agents/constraints/*.
...
try:
    ...
    from .project_profile import detect as detect_profile
...
_ROADMAP_RESIDUE_RULES: List[tuple[str, re.Pattern[str], str, str]] = [
    (
        ...
        "Keep roadmap labels inside agent_roadmaps/ only, or remove the "
        "temporary roadmap reference.",
    ),
]
...
def main() -> None:
    ...
    if args.project_type == "auto":
        ...
        is_hybrid = bool(profile and profile.is_hybrid())
        project_type = ProjectType.UNKNOWN if is_hybrid else detect(repo_root)
        reported_type = "hybrid" if is_hybrid else project_type.value
    else:
        project_type = ProjectType(args.project_type)
        reported_type = project_type.value
