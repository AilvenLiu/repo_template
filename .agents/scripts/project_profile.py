...
def _read_profile_from_yml(repo_root: Path) -> Optional[ProjectProfile]:
    """Read project_profile from .agents/project.yml."""
    yml_path = repo_root / ".agents" / "project.yml"
...
def _read_legacy_project_type(repo_root: Path) -> Optional[str]:
    """Read legacy project_type from .agents/project.yml."""
    yml = repo_root / ".agents" / "project.yml"
    if not yml.exists():
        return None
