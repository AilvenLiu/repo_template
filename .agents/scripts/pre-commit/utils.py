...
# Use shared detection from .agents/scripts/project_type.py
...
class PreCommitManager:
    ...
    def is_template_repository(self) -> bool:
        """Return whether this is the template infrastructure repository."""
        return (self.repo_root / ".agents" / "skills" / "create-project").is_dir() and (
    ...
    def python_environment_status(self, profile: ProjectProfile) -> tuple[bool, str]:
        ...
        if self.is_template_repository() and not pyproject.exists():
            return (
                True,
                "Template infrastructure repository (no application environment)",
            )
        ...
        if venv_exists:
            venv_name = next(
                venv for venv in venv_paths if (self.repo_root / venv).exists()
            )
    ...
    def python_dependency_manifest_status(
