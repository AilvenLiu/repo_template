...
class ConstraintViolation:
    def __init__(
        self, category: str, severity: str, message: str, detail: str, remediation: str
    ):
...
def _is_template_repository(repo_root: Path) -> bool:
    """Return whether cwd is the source template, not a generated project."""
    return (repo_root / ".agents" / "skills" / "create-project").is_dir() and (
        repo_root / "templates"
    ).is_dir()
...
def check_python_dependency_constraints(
    ...
    scikit_build_project = (
        profile is not None
        and profile.build_system
        in (BuildSystem.SCIKIT_BUILD, BuildSystem.SCIKIT_BUILD_CORE)
    ...
    if (
        ...
        and not poetry_lock_exists
        and not scikit_build_project
    ):
        violations.append(
            ConstraintViolation(
                ...
                remediation=(
                    "Run: .agents/bin/agent-python-env-setup fix, then poetry install. "
                    "For hybrid projects, validate direct CMake before the documented bridge."
                ),
            )
        )
...
def validate_all_constraints() -> List[ConstraintViolation]:
    ...
    if (
        os.path.exists("pyproject.toml") or os.path.exists("requirements.txt")
    ) and not _is_template_repository(Path.cwd()):
