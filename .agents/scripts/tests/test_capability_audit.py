...
class TestWhenSelectorEvaluation:
    ...
    def test_build_system_in_list(self):
        ...
        assert (
            _evaluate_when_selector("build_system in [cmake, bazel]", profile) is True
        )
        assert (
            _evaluate_when_selector("build_system in [poetry, scikit-build]", profile)
            is False
        )
    ...
    def test_hardware_targets_in_list(self):
        ...
        assert (
            _evaluate_when_selector("hardware_targets in [cuda, rocm]", profile) is True
        )
        assert (
            _evaluate_when_selector("hardware_targets in [rocm, webgpu]", profile)
            is False
        )
    ...
    def test_or_selector(self):
        ...
        assert (
            _evaluate_when_selector(
                "distribution=pypi-wheel OR hardware_targets=cuda", profile
            )
            is True
        )
    ...
    def test_and_selector(self):
        ...
        assert (
            _evaluate_when_selector("language=python AND build_system=poetry", profile)
            is True
        )
        assert (
            _evaluate_when_selector("language=python AND build_system=cmake", profile)
            is False
        )
...
class TestAuditFixtures:
    ...
    def test_hybrid_project_enables_both_languages(self, tmp_path):
        ...
        # Create a project with explicit hybrid profile in .agents/project.yml
