...
Usage:
    python3 .agents/scripts/context7/check_mcp.py
"""
...
def check_mcp_server():
    ...
    try:
        result = subprocess.run(
            ["claude", "plugins", "list"], capture_output=True, text=True, check=False
        )
    ...
    try:
        ...
        if (
            "plugin:context7:context7" in result.stdout
            or "context7" in result.stdout.lower()
        ):
