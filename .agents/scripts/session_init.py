...
def find_active_roadmap(repo_root: Path) -> Optional[Path]:
    ...
    for roadmap_file in sorted(roadmaps_dir.rglob("roadmap.yml")):
        ...
        if isinstance(parsed, dict):
            ...
            if isinstance(status, dict):
                ...
                if isinstance(active_flag, bool):
                    ...
                    is_active = active_flag.strip().lower() in {
                        "true",
                        "active",
                        "in_progress",
                    }
        ...
        if not is_active and (
            "status: active" in content or "status: in_progress" in content
        ):
            is_active = True
...
def load_constraint(repo_root: Path, key: str) -> Optional[str]:
    path = repo_root / ".agents" / "constraints" / f"{key}.md"
    if not path.exists():
        return None
    try:
...
def run_init(platform: str, verbose: bool = False) -> int:
    ...
    project_yml = repo_root / ".agents" / "project.yml"
    source = ".agents/project.yml" if project_yml.exists() else "heuristic"
    ...
    if branch:
        if branch in PROTECTED_BRANCHES or any(
            branch.startswith(p) for p in PROTECTED_PREFIXES
        ):
    ...
    for key in keys:
        ...
        if effective_verbose:
            ...
            print(f"[READ] .agents/constraints/{key}.md")
