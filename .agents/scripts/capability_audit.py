    ...
    try:
        ...
        if "common_requirements:" in text or "platform_requirements:" in text:
            raise RuntimeError(
                ...
                "Declare PyYAML through the repository dependency workflow and rerun Poetry setup"
            )
        if re.search(r"^\s*[^#\n]+:\s*[>|]", text, re.MULTILINE):
            print(
...
def _is_template_repo(repo_root: Path) -> bool:
    return (repo_root / ".agents" / "skills" / "create-project").is_dir()
...
def _evaluate_when_selector(selector: str, profile: Any) -> bool:
    ...
    if " OR " in selector:
        return any(
            _evaluate_when_selector(part, profile) for part in selector.split(" OR ")
        )

    if " AND " in selector:
        return all(
            _evaluate_when_selector(part, profile) for part in selector.split(" AND ")
        )
...
def _entry_enabled_for_repo(
    entry: Dict[str, Any], is_template: bool, repo_root: Path
) -> bool:
...
def _audit_project_skills(
    ...
    """Audit vendor-neutral skill manifests under .agents/skills/<id>/SKILL.md.
    ...
    for entry in entries:
        ...
        agents_ok = agents_skill_md.is_file()
        claude_ok = (not is_claude_platform) or claude_skill_md.is_file()

        available = agents_ok and claude_ok
        message_parts: List[str] = []
        if not agents_skill_md.is_file():
            message_parts.append(
                f"Skill body missing: .agents/skills/{skill_id}/SKILL.md"
            )
        ...
        method = "native Codex discovery (.agents/skills/<id>/SKILL.md"
...
def _audit_claude_marketplaces(
    entries: List[Dict[str, Any]], result: AuditResult
) -> None:
...
def _audit_claude_plugin_skills(
    ...
    for plugin in plugins:
        ...
        if plugin_id and install_path:
            plugin_skill_map[plugin_id] = _discover_plugin_skills(
    ...
    for entry in entries:
        ...
        if not available:
            if plugin_id not in plugin_skill_map:
                ...
                message = f"Skill '{skill_id}' not provided by plugin '{plugin_id}'."
            ...
            if integration_id == "context7-mcp":
                ...
                result.add(
                    AuditEntry(
                        ...
                        available=configured,
                        method="claude mcp list + plugin fallback",
                        message="No Context7 entry found in `claude mcp list`. "
                        + plugin_message,
                    )
                )
                continue
...
def run_audit(
    ...
    manifest = manifest_path or repo_root / ".agents" / "capabilities.yml"
    ...
    platform_req = normalized.get("platform_requirements", {}).get(platform, {}) or {}

    result = AuditResult(platform=platform)
    is_template = _is_template_repo(repo_root)

    _audit_project_skills(
        common.get("project_skills", []), repo_root, is_template, result
    )
    ...
    if platform == "claude":
        ...
        _audit_claude_plugin_skills(
            platform_req.get("claude_plugin_skills", []), result
        )
    ...
    for category in sorted(grouped.keys()):
        ...
        for entry in grouped[category]:
            status = (
                "[OK]"
                if entry.available
                else ("[FAIL]" if entry.required else "[MISS]")
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--manifest",
        default=".agents/capabilities.yml",
        help="Path to capabilities manifest (default: .agents/capabilities.yml)",
    )
    parser.add_argument(
